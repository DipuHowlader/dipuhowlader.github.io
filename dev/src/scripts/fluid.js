(function (w) {
  ...
  /*
        ...
        */
  var canvas_width = 1300; //Needs to be a multiple of the resolution value below.
  ...
  function init() {
    ...
    for (i = 0; i < speck_count; i++) {
      ...
      particles.push(
        new particle(
          Math.random() * 100 * canvas_width,
          Math.random() * 100 * canvas_height
        )
      );
    }
    ...
    try {
      document.querySelector(".form").addEventListener("mousedown", (e) => {
        clicking = false;
      });

      w.addEventListener("mouseup", () => (clicking = true));
